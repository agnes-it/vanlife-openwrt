...
define(["text!temple/upgrade/index.html", "css!temple/upgrade/index.css", "component/gl-btn/index", "component/gl-toggle-btn/index", "component/gl-upload-file/index", "component/gl-select/index", "vue", 'Cookie', "component/gl-tooltip/index", "component/modal/modal"], function (stpl, css, gl_btn, gl_toggle, gl_upf, gl_select, Vue, Cookies, gl_tooltip, gl_modal) {
    var vueComponent = Vue.extend({
        ...
        computed: {
            isTo4() {
                const toV =this.upgradeVersionVal.toString().replace(/[^\d]/g,'').trim()[0]
                return Number(toV)>=4
            },
        ...
        methods: {
            ...
            upgradeTo4(){
                this.$store.commit("showModal", {
                    show: true,
                    title: "Caution",
                    message: this.t("When upgrading the firmware from version 3.x to 4.x it is NOT possible to keep all data. Your Admin Password, Wireless, LAN, Cable and Modem Settings will be kept, but ALL other settings and user data will be DELETED."),
                    messageTwo:this.t('Do you still want to install?'),
                    type: "warning",
                    yes:"install",
                    no:'Cancel',
                    cb:function cb() {   
                        if (window.caniuse) {
                            ...
                            sessionStorage.setItem("keepapps", false);
                            sessionStorage.setItem("callfunction", "upgradefirmware");
                        }
                        if (window.location.href.indexOf('chrome-extension') == -1) {
                            window.location.href = "/#/process?action=upgradefirmware&keepsetting=" + this.KeepSettingBtn + "&keepapps=" + false;
                        } else {
                            window.location.href = "/idx_vue.html#/process?action=upgradefirmware&keepsetting=" + this.KeepSettingBtn + "&keepapps=" + false;
                        }
                     }
                })
            },
            ...
            installBtnClick: function installBtnClick() {
                if (this.isTo4 && this.KeepSettingBtn) {
                    this.upgradeTo4()
                    return
                }
                var that = this;
